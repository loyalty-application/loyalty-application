...
resource "aws_s3_object" "this" {
  ...
  key    = "kafka-connect-spooldir-2.0.65.jar"
  source = "./kafka-connect-spooldir-2.0.65.jar"
}

# create plugin with the connector
resource "aws_mskconnect_custom_plugin" "this" {
  name         = "${local.project_name}-custom-plugin"
  content_type = "JAR"
  location {
    s3 {
...
offset.storage.topic=docker-connect-offsets #https://docs.aws.amazon.com/msk/latest/developerguide/msk-connect-workers.html -> specific guide
